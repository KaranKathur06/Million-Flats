  ...
  try {
    const response = await fetch(`${META_API_BASE}/${phoneNumberId}/messages`, {
      ...
      body: JSON.stringify({
        ...
        type: 'template',
        template: {
          name: process.env.META_WHATSAPP_AUTH_TEMPLATE_NAME || 'mf_authentication_otp',
          language: { code: 'en' },
          components: [
            {
              type: 'body',
...
/** Sends the one-time welcome message after first successful login */
export async function sendWelcomeMessage(phone: string): Promise<MetaMessageResult> {
  const { accessToken, phoneNumberId } = getConfig()
  if (!accessToken || !phoneNumberId) return { success: false, error: 'API_NOT_CONFIGURED' }
...
/** Generates the pre-filled WhatsApp deeplink URLs */
export function generateWhatsAppDeeplink(sessionId: string): { mobile: string; web: string } {
  const { businessPhone } = getConfig()
  const cleanPhone = businessPhone.replace(/[^0-9]/g, '')

  const message = `Securely log me into MillionFlats.\n[Code: ${sessionId}]\nBy sending this message, I request instant unrestricted access, and agree to receive premium property updates.`
  const encodedMessage = encodeURIComponent(message)

  return {
    mobile: `https://wa.me/${cleanPhone}?text=${encodedMessage}`,
...
/** Extracts session ID from a WhatsApp message text */
export function extractSessionIdFromMessage(text: string): string | null {
  const match = text.match(/\[Code:\s*(MF-\d{4}-[A-Z0-9]{8})\]/)
