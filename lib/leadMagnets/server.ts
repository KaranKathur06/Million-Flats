...
export async function createLeadMagnetDownload(params: {
  ...
  // Use CloudFront signed URL instead of raw S3 presigned URL
  const signed = await generateSignedUrl({ s3Key: reachableKey, ttlSeconds: ASSET_TTL.PROTECTED_DOWNLOAD })

  // Track access in asset access log (non-blocking)
  trackAssetAccess({
