...
export const developerImportAdapter: ImportAdapter<CanonicalDeveloper> = {
  ...
  normalize(input: NormalizationInput): NormalizationResult {
    ...
    const city = text(read(raw, ['city', 'location.city']))
    const countryResolution = resolveLocationCountry({ country: read(raw, ['countryIso2', 'country_iso2', 'country_code', 'country']), city, community: read(raw, ['community', 'locality', 'neighborhood']), currency: read(raw, ['currency', 'priceCurrency', 'price_currency']), address: read(raw, ['address', 'office_address', 'headquarters_address']) })
    const countryIso2 = countryResolution.countryIso2
    const normalized = { ...raw, name, slug: text(read(raw, ['slug', 'developer_slug'])) || (name ? slugify(name) : null), countryIso2, countryCode: countryIso2 === 'IN' ? 'INDIA' : countryIso2 === 'AE' ? 'UAE' : null, city, website: text(read(raw, ['website', 'website_url', 'url'])), shortDescription: text(read(raw, ['shortDescription', 'short_description', 'tagline'])), description: text(read(raw, ['description', 'full_description', 'about'])), foundedYear: Number(read(raw, ['foundedYear', 'founded_year', 'year_founded'])) || null, email: text(read(raw, ['email', 'email_address', 'contact_email']))?.toLowerCase(), phone: text(read(raw, ['phone', 'phone_number', 'contact_phone'])), address: text(read(raw, ['address', 'office_address', 'headquarters_address'])), metaTitle: text(read(raw, ['metaTitle', 'meta_title', 'seo_title'])), metaDescription: text(read(raw, ['metaDescription', 'meta_description', 'seo_description'])), metaKeywords: text(read(raw, ['metaKeywords', 'meta_keywords', 'seo_keywords'])), sourceState: text(read(raw, ['state', 'province', 'region'])), sourceDuplicateCheck: text(read(raw, ['duplicateCheck', 'duplicate_check'])), isFeatured: normalizeBoolean(read(raw, ['isFeatured', 'is_featured'])), status: String(read(raw, ['status']) || 'ACTIVE').toUpperCase() === 'INACTIVE' ? 'INACTIVE' : 'ACTIVE' }
    return { normalized, warnings: countryResolution.needsReview ? [countryResolution.reason] : [], errors: [] }
  },
  ...
  async commit(input) {
    ...
    const data = { name: value.name, slug, countryCode: value.countryCode || null, countryIso2: value.countryIso2 || null, city: value.city || null, description: value.description || null, shortDescription: value.shortDescription || null, website: value.website || null, foundedYear: value.foundedYear ?? null, email: value.email || null, phone: value.phone || null, address: value.address || null, metaTitle: value.metaTitle || null, metaDescription: value.metaDescription || null, metaKeywords: value.metaKeywords || null, isFeatured: Boolean(value.isFeatured), featuredRank: value.featuredRank ?? null, status: value.status || 'ACTIVE' }
