...
// Only government identity proof is required for onboarding. Professional/proof licenses are optional trust enhancers.
const REQUIRED_DOCS = [
  {
    type: 'GOVERNMENT_ID',
    label: 'Government Identity Proof',
    desc: 'Passport, Emirates ID, Aadhaar, Driving License, National ID or equivalent',
    icon: (
...
const OPTIONAL_DOCS = [
  {
    ...
    label: 'Real Estate / Professional License',
    desc: 'RERA, Broker License, Agency Registration, or equivalent (optional)',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function AgentVerificationCenter({
  ...
  const profileRequirements = useMemo(() => {
    ...
    // License is optional now; require phone, photo and bio length
  ...
  const alreadySubmitted = pStatus === 'SUBMITTED' || pStatus === 'VERIFIED'
