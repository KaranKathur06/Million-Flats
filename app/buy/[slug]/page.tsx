...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const property = await requirePublicManualProperty(params.slug, 'SALE')
  const title = safeString(property?.title) || 'Property for Sale'
  ...
  return {
    title: `${title}${locationLabel ? ` in ${locationLabel}` : ''} | Buy | millionflats`,
    description,
...
export default async function BuyPropertyDetailPage({ params }: { params: { slug: string } }) {
  const property = await requirePublicManualProperty(params.slug, 'SALE')
