...
export default function AdminShellHeaderClient() {
  ...
  return (
    <div className="flex h-full items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
      {/* Workspace identity + sidebar toggle */}
      <div className="flex min-w-0 items-center gap-3 sm:gap-4">
        <SidebarToggleButton collapsed={sidebarCollapsed} onClick={toggleSidebar} className="hidden md:inline-flex" />

        <div className="hidden md:block h-9 w-px bg-white/[0.08] shrink-0" aria-hidden />

        <div className="relative flex h-11 w-auto aspect-video shrink-0 px-2 overflow-hidden items-center justify-center rounded-xl bg-white border border-white/[0.08] shadow-lg shadow-amber-500/10 ring-1 ring-black/40">
          <img src="/LOGO.jpeg" alt="MillionFlats" className="h-full w-full object-contain" />
        </div>

        <div className="min-w-0 flex flex-col justify-center">
          <div className="truncate text-[17px] sm:text-[18px] font-extrabold tracking-tight text-white/95 leading-tight">
            MillionFlats
          </div>
          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5">
            <span className="inline-flex h-[20px] items-center rounded-md bg-gradient-to-r from-amber-400/20 to-amber-500/10 px-2 text-[10px] font-bold uppercase tracking-[0.1em] text-amber-400 border border-amber-400/20">
              {roleLabel}
            </span>
            <span className="text-[11px] font-bold text-white/45 tracking-[0.12em] uppercase hidden sm:inline">
              Workspace
            </span>
          </div>
        </div>
