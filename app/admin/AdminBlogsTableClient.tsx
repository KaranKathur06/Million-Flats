    ...
    <>
      ...
      <div>
        ...
        {blogItems.length === 0 ? (
          ...
          <ResponsiveDataTable
            mobileCards={blogItems.map((item) => {
              const statusCfg = getStatusConfig(item.status)
              const seoCfg = getSEOScoreColor(item.seoScore)
              return (
                <AdminDataCard
                  key={item.id}
                  selected={selectedBlogs.includes(item.id)}
                  onSelect={() => handleSelectOne(item.id)}
                  title={
                    <Link href={`/admin/blogs/${item.id}/edit`} className="hover:text-amber-300 transition-colors">
                      {item.title}
                    </Link>
                  }
                  subtitle={item.slug}
                  status={
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide rounded-lg border ${statusCfg.bg} ${statusCfg.text} ${statusCfg.border}`}>
                  ...
                  meta={
                    <>
                      <p><span className="text-white/35">Category:</span> {item.categoryName}</p>
                      <p><span className="text-white/35">Author:</span> {item.authorName}</p>
                      <p><span className="text-white/35">SEO:</span> <span className={seoCfg.text}>{item.seoScore}</span></p>
                      <p><span className="text-white/35">Date:</span> {formatDate(item.publishAt || item.createdAt)}</p>
                      <p><span className="text-white/35">Views:</span> {item.views.toLocaleString()}</p>
                    </>
                  }
                  actions={
                    <div className="flex flex-wrap items-center gap-2">
                      <Link
                        ...
                        className="mf-touch-target inline-flex items-center rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-white/80"
                      >
                        Edit
                      </Link>
                      <button
                        ...
                        className="mf-touch-target inline-flex items-center rounded-lg border border-blue-400/20 bg-blue-500/10 px-3 py-1.5 text-xs font-medium text-blue-200 disabled:opacity-50"
                      >
                        Duplicate
                      </button>
                      <button
                        ...
                        className="mf-touch-target inline-flex items-center rounded-lg border border-red-400/20 bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-200"
                      >
                        Delete
                      </button>
                    </div>
