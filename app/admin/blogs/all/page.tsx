...
export default async function AllBlogsPage({
  ...
  if (!session?.user) {
    redirect('/user/login?next=%2Fadmin%2Fblogs%2Fall')
  }
    ...
    <div className="mx-auto max-w-[1500px] space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight">All Blogs</h1>
          <p className="mt-1 text-sm text-white/50">View, filter, and manage all blog posts</p>
        </div>
        <Link
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          return (
            <Link
              key={tab.value}
              href={tab.value ? `/admin/blogs/all?status=${tab.value}` : '/admin/blogs/all'}
