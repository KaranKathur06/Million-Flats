...
export const metadata: Metadata = { title: 'Agency Profiles | MillionFlats Admin' }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Agency Profiles</h1>
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1 flex-wrap">
        {STATUS_FILTERS.map(f => (
          <Link
            key={f.value}
            href={`/admin/agencies?status=${f.value}${q ? `&q=${encodeURIComponent(q)}` : ''}`}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              {['Agency Name', 'Email', 'Status', 'KYC', 'Completion', 'Linked', 'Joined', ''].map(h => (
        ...
        {total > limit && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <Link
                  href={`/admin/agencies?status=${status}&page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ''}`}
