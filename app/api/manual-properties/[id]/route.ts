...
const PaymentPlanStageSchema = z.object({
  id: z.string().trim().min(1).max(80), label: z.string().trim().max(120),
  type: z.string().optional(), basis: z.enum(['PERCENTAGE', 'FIXED_AMOUNT']).optional(),
  percentage: z.number().finite().min(0).max(100).optional().nullable(), fixedAmount: z.number().finite().min(0).optional().nullable(),
  ...
  milestone: z.string().max(300).optional().nullable(), description: z.string().trim().max(300).optional().nullable(), order: z.number().int().min(0).max(1000),
})
