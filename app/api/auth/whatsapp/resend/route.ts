...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session) {
      return NextResponse.json(
        { message: 'Session not found or expired.', error: 'SESSION_NOT_FOUND' },
        { status: 400 },
      )
    }
    ...
    if (session.status === 'FAILED' || new Date() > new Date(session.expiresAt)) {
      return NextResponse.json(
        { message: 'Session expired. Please start again.', error: 'SESSION_EXPIRED' },
        { status: 400 },
      )
    }
    ...
    if (!sendResult.success) {
      await logWhatsAppEvent({
        ...
        response: { event: 'otp_resend_failed' },
      })
      return NextResponse.json(
        { message: 'Failed to resend code. Please try again.', error: 'META_API_ERROR' },
        { status: 503 },
      )
    }
