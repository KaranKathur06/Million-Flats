...
export async function GET(_req: Request, { params }: { params: { slug: string } }) {
    try {
        ...
        const heroFallback = media.hero[0]?.url || buildAssetUrl(project.coverImage) || FALLBACK_IMAGE
        const normalizedUnitTypes = (project.unitTypes || []).map((ut: any) => {
            ...
            return {
                ...
                const floorPlans = (variant.floorPlans || []).map((fp: any) => ({
                    ...
                    image_url: buildAssetUrl(fp.imageUrl) || FALLBACK_IMAGE,
                    size: fp.size || null,
        ...
        // Apply CDN URLs to similar projects and developer
        const resolvedSimilar = (similarProjects || []).map((sp: any) => ({
            ...sp,
            coverImage: buildAssetUrl(sp.coverImage) || FALLBACK_IMAGE,
        }))
        ...
        return NextResponse.json({
            ...
            project: {
                ...
                developer: project.developer ? {
                    ...project.developer,
                    logo: buildAssetUrl(project.developer.logo),
                } : null,
            },
            media,
