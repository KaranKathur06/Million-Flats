...
export async function POST(req: Request) {
  try {
    ...
    const updated = await (prisma as any).agent.update({
      ...
      data: {
        profilePhoto: uploaded.key,
        profileImageUrl: uploaded.key,
    ...
    // Return CDN URL in the response for immediate UI update
    const responseAgent = {
      ...updated,
      profilePhoto: buildAssetUrl(updated.profilePhoto),
      profileImageUrl: buildAssetUrl(updated.profileImageUrl),
    }
    ...
    return NextResponse.json({ success: true, agent: responseAgent }, { status: 200 })
  } catch (error) {
