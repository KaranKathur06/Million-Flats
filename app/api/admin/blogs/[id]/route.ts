...
import { normalizeBlogAssets } from '@/lib/blogs/normalizeBlogAssets'
...
function isAllowedBlogImageUrl(value: string) {
  ...
  try {
    ...
    // Accept CDN domain
    const cdnDomain = safeString(process.env.NEXT_PUBLIC_CDN_DOMAIN).replace(/^https?:\/\//, '').replace(/\/$/, '')
    if (cdnDomain && parsed.hostname === cdnDomain) return true
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: normalizeBlogAssets(blog) })
  } catch (error) {
  ...
  try {
    ...
    const updatedBlog = await (prisma as any).blog.update({
      ...
      data: {
        ...
        featuredImageUrl: safeString(body.featuredImageUrl) || null,
        featuredImageAlt: safeString(body.featuredImageAlt) || null,
