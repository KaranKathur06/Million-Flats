...
export default function AuthDeveloperLoginClient() {
  ...
  const callbackUrl = safeNext ? `/auth/redirect?next=${encodeURIComponent(safeNext)}` : '/developer/dashboard'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (raw === 'DEVELOPER_NOT_REGISTERED') setError('Developer account not found. Please register first.')
      else setError(raw)
    } catch {
  ...
  return (
    <AuthLayout title="Developer Sign In" subtitle="Grow your projects on MillionFlats">
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {error === 'Please verify your email before signing in.' && (
          <Link
            href={`/developer/verify-otp?email=${encodeURIComponent(email)}`}
        ...
        {error === 'Password is not set for this account. Please reset your password.' && (
          <Link
            href={`/auth/developer/forgot-password?email=${encodeURIComponent(email)}`}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Link
              href={`/auth/developer/forgot-password${email ? `?email=${encodeURIComponent(email)}` : ''}`}
        ...
        <p className="text-center text-sm text-gray-600">
          ...
          <Link href="/developer/register" className="font-medium text-blue-600 hover:text-blue-700">
            Register here
          </Link>
        </p>
      </form>
