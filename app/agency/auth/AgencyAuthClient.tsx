...
function LoginTab() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {showLoginResetCta ? (
        <Link href={`/agency/forgot-password?email=${encodeURIComponent(email)}`} className="block w-full rounded-xl bg-amber-400 text-white px-4 py-3 text-center text-sm font-semibold hover:shadow-lg">
          Reset Password
        </Link>
      ) : null}
