...
export function PaymentPlan({ stages, price, currency = 'AED', areaSquareFeet }: { stages: unknown; price?: number | null; currency?: string | null; areaSquareFeet?: number | null }) {
  ...
  return (
    <section className="bg-white rounded-3xl p-5 md:p-7 shadow-sm">
      ...
      {calculation.oneTimeCostDetails.length > 0 ? <div className="mt-4 rounded-2xl border border-gray-200 bg-gray-50 p-4"><p className="text-xs font-semibold uppercase tracking-widest text-gray-500">Estimated purchase costs</p><div className="mt-3 space-y-2 text-sm text-gray-700">{calculation.oneTimeCostDetails.map((cost) => <div key={cost.id} className="flex items-center justify-between gap-4"><span>{cost.label || cost.category}</span><strong className="text-dark-blue">{formatFinancialAmount(cost.calculatedAmount, safeCurrency)}</strong></div>)}<div className="flex items-center justify-between gap-4 border-t border-gray-200 pt-3 font-semibold text-dark-blue"><span>Estimated property price + one-time costs</span><span>{formatFinancialAmount(calculation.propertyPrice + calculation.oneTimeCosts, safeCurrency)}</span></div></div><p className="mt-2 text-xs text-gray-500">Property price remains {formatFinancialAmount(calculation.propertyPrice, safeCurrency)}. Recurring charges and financing are not included in this estimate.</p></div> : null}
    </section>
  )
}