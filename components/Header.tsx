...
export default function Header() {
  ...
  return (
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isAuthLoading ? null : !isAuthed ? (
              <>
                ...
                {/* Dynamic auth button based on admin-controlled auth mode */}
                {authConfig.activeMode === 'WHATSAPP_ONLY' && (
                  <button
                    type="button"
                    onClick={() => setWaModalOpen(true)}
                    className="inline-flex items-center gap-2 bg-[#25D366] text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-[#20BD5A] transition-colors shadow-sm"
                  >
                ...
                {authConfig.activeMode === 'EMAIL_ONLY' && (
                  <Link
                    href="/auth/login"
                    className="inline-flex items-center gap-2 bg-dark-blue text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-dark-blue/90 transition-colors"
                  >
                    Sign In
                  </Link>
                )}
                {authConfig.activeMode === 'EMAIL_AND_WHATSAPP' && (
                  <>
                    <button
                      type="button"
                ...
                {authConfig.activeMode === 'DISABLED' && (
                  <span className="text-sm text-gray-400 italic">Login unavailable</span>
                )}
              </>
            ) : (
              <>
                ...
                <>
                  ...
                  {authConfig.activeMode === 'WHATSAPP_ONLY' && (
                    <button
                      ...
                      onClick={() => { setMobileOpen(false); setWaModalOpen(true) }}
                    >
                      Login with WhatsApp
                    </button>
                  )}
                  {authConfig.activeMode === 'EMAIL_ONLY' && (
                    <Link
                      href="/auth/login"
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-white bg-dark-blue"
                      onClick={() => setMobileOpen(false)}
                    >
                      Sign In
                    </Link>
                  ...
                  {authConfig.activeMode === 'EMAIL_AND_WHATSAPP' && (
                    <button
                      type="button"
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-white bg-[#25D366]"
                      onClick={() => { setMobileOpen(false); setDualModalOpen(true) }}
                    >
                      Login with WhatsApp
                    </button>
                  )}
                  {authConfig.activeMode === 'DISABLED' && (
                    <span className="block px-4 py-3 text-sm text-gray-400 italic text-center">Login unavailable</span>
                  )}
                </>
              ) : (
                <>
    ...
    {/* Auth Modals — rendered as portals outside the header */}
    <WhatsAppLoginModal open={waModalOpen} onClose={() => setWaModalOpen(false)} />
    <DualAuthModal open={dualModalOpen} onClose={() => setDualModalOpen(false)} />
    </>
  );
}
