...
export function MediaUploadDialog({ entityId, title, categories, initialCategory, presignEndpoint, finalizeEndpoint, buildPresignBody, buildFinalizeBody, isOpen, onClose, onComplete }: {
  entityId: string
  title: string
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="relative w-full max-w-2xl rounded-2xl border border-white/[0.08] bg-[#0b1420] p-6 shadow-2xl"><div className="mb-6 flex items-center justify-between"><div><h2 className="text-xl font-semibold text-white">Upload {title} Media</h2><p className="mt-1 text-xs text-white/40">Upload files first, then manage categories in the library.</p></div><button type="button" onClick={onClose} aria-label="Close" className="text-white/50 hover:text-white">×</button></div><div className="mb-5"><label className="mb-2 block text-xs font-semibold uppercase tracking-wider text-white/40">Category for this batch</label><GlobalDropdown value={category} onChange={(value) => setCategory(String(value))} options={categories} appearance="admin-dark" showLabel={false} /></div>{files.length === 0 ? <div onDragEnter={(event) => { event.preventDefault(); setDragging(true) }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); add(Array.from(event.dataTransfer.files)) }} className={`rounded-xl border-2 border-dashed p-10 text-center ${dragging ? 'border-amber-400/60 bg-amber-400/10' : 'border-white/[0.12] bg-white/[0.02]'}`}><p className="text-sm font-medium text-white/70">Drop media here</p><p className="mt-1 text-xs text-white/40">or choose multiple files</p><button type="button" onClick={() => inputRef.current?.click()} className="mt-4 rounded-lg bg-amber-400/20 px-4 py-2 text-sm font-medium text-amber-300">Browse files</button><input ref={inputRef} type="file" multiple accept="image/*,application/pdf" className="hidden" onChange={(event) => { add(Array.from(event.target.files || [])); event.currentTarget.value = '' }} /></div> : <div className="max-h-64 space-y-2 overflow-y-auto">{files.map((file) => <UploadRow key={file.id} file={file} onRemove={() => removeFile(file.id)} onRetry={() => retryFile(file.id)} />)}</div>}<div className="mt-6 flex justify-between"><button type="button" onClick={onClose} className="rounded-lg border border-white/[0.08] px-4 py-2 text-sm text-white/60">Close</button>{complete ? <button type="button" onClick={() => { onClose() }} className="rounded-lg bg-emerald-400/20 px-4 py-2 text-sm text-emerald-300">Done</button> : null}</div></div></div>
}
