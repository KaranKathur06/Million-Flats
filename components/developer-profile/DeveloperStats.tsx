...
export default function DeveloperStats({ developer }: DeveloperStatsProps) {
  ...
  const priceRange = buildDeveloperPriceRange({
    minPrice: stats.startingPriceMin,
    maxPrice: stats.startingPriceMax ?? stats.startingPriceMin,
    sourceCurrency: stats.startingPriceCurrency,
    displayCurrency,
  })
          ...
          {priceRange && (
            <div className="mt-4 rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-amber-800">{priceRange}</p>
              </div>
            </div>
          )}
