...
import { PROPERTY_MEDIA_CATEGORIES, PROPERTY_MEDIA_CATEGORY_OPTIONS, type PropertyMediaCategory } from '@/lib/propertyMedia'
import { useAdminAction } from '@/components/admin/AdminActionProvider'

type Media = { id: string; category: PropertyMediaCategory; url: string; altText?: string | null; position: number; floorPlanTitle?: string | null; floorPlanBedroomCount?: number | null }

const labels = Object.fromEntries(PROPERTY_MEDIA_CATEGORY_OPTIONS.map((option) => [option.value, option.label])) as Record<PropertyMediaCategory, string>
...
export function PropertyMediaManager({ propertyId }: { propertyId: string }) {
  ...
  const [uploadCategory, setUploadCategory] = useState<PropertyMediaCategory>('EXTERIOR')
  ...
  async function upload(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        const floorPlanData = uploadCategory === 'FLOOR_PLANS' && floorPlanBedroomCount.trim() ? { floorPlanTitle: file.name, floorPlanBedroomCount: Number(floorPlanBedroomCount) } : {}
        const presign = await fetch(`/api/admin/properties/${propertyId}/media`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: file.name, fileSizeBytes: file.size, contentType: file.type, category: uploadCategory, ...floorPlanData }) })
        ...
        const finalize = await fetch(`/api/admin/properties/${propertyId}/media`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: file.name, fileSizeBytes: file.size, contentType: file.type, category: uploadCategory, s3Key: signed.s3Key, ...floorPlanData }) })
  ...
  const filteredMedia = selectedCategory ? media.filter((item) => item.category === selectedCategory) : media
  ...
  return <section className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 space-y-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <GlobalDropdown value={uploadCategory} onChange={(value) => setUploadCategory(String(value) as PropertyMediaCategory)} options={PROPERTY_MEDIA_CATEGORY_OPTIONS} appearance="admin-dark" dense showLabel={false} className="min-w-40" />
    ...
    {filteredMedia.length === 0 ? <div className="rounded-xl border border-dashed border-white/[0.12] p-7 text-center text-sm text-white/35">No media uploaded yet.</div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{filteredMedia.map((item) => { const index = media.findIndex((candidate) => candidate.id === item.id); return <figure key={item.id} className="overflow-hidden rounded-xl border border-white/[0.08] bg-black/20"><img src={item.url} alt={item.altText || 'Property media'} className="aspect-square w-full object-cover" /><figcaption className="space-y-2 p-2"><div className="flex items-center justify-between gap-2"><p className="text-xs text-white/55">{labels[item.category]}</p>{item.category === 'COVER' ? <span className="text-[10px] font-semibold text-amber-300">★ Hero</span> : null}</div>{item.category === 'FLOOR_PLANS' ? <p className="text-[10px] text-white/45">{item.floorPlanTitle || 'Floor plan'}{item.floorPlanBedroomCount !== null && item.floorPlanBedroomCount !== undefined ? ` · ${item.floorPlanBedroomCount} Bedroom` : ''}</p> : null}<GlobalDropdown value={item.category} onChange={(value) => void updateMedia(item.id, { category: String(value) })} options={PROPERTY_MEDIA_CATEGORY_OPTIONS} appearance="admin-dark" dense showLabel={false} /><div className="flex flex-wrap gap-1"><button type="button" onClick={() => void updateMedia(item.id, { category: 'COVER' })} disabled={item.category === 'COVER'} className="rounded bg-amber-400/15 px-2 py-1 text-[10px] text-amber-300 disabled:opacity-30">Set as Hero</button><button type="button" disabled={index === 0} onClick={() => void move(item.id, -1)} className="rounded bg-white/[0.07] px-2 py-1 text-[10px] text-white/65 disabled:opacity-30">←</button><button type="button" disabled={index === media.length - 1} onClick={() => void move(item.id, 1)} className="rounded bg-white/[0.07] px-2 py-1 text-[10px] text-white/65 disabled:opacity-30">→</button><button type="button" onClick={() => void removeMedia(item.id)} className="rounded bg-red-400/15 px-2 py-1 text-[10px] text-red-300">Delete</button></div></figcaption></figure> })}</div>}
  </section>
}
