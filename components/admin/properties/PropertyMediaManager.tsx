...
export function PropertyMediaManager({ propertyId }: { propertyId: string }) {
  ...
  return <section className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 space-y-4">
    ...
    {filteredMedia.length === 0 ? <div className="rounded-xl border border-dashed border-white/[0.12] p-7 text-center text-sm text-white/35">No media uploaded yet.</div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{filteredMedia.map((item) => { const index = media.findIndex((candidate) => candidate.id === item.id); return <MediaCard key={item.id} media={{ id: item.id, mediaUrl: item.url, category: item.category, label: item.category === 'FLOOR_PLANS' ? `${item.floorPlanTitle || 'Floor plan'}${item.floorPlanBedroomCount !== null && item.floorPlanBedroomCount !== undefined ? ` · ${item.floorPlanBedroomCount} Bedroom` : ''}` : labels[item.category], altText: item.altText }} appearance="dark" isHero={item.category === 'COVER'} onSetAsHero={() => void updateMedia(item.id, { category: 'COVER' })} onMoveUp={index > 0 ? () => void move(item.id, -1) : undefined} onMoveDown={index < media.length - 1 ? () => void move(item.id, 1) : undefined} onDelete={() => void removeMedia(item.id)} /> })}</div>}
  </section>
}
