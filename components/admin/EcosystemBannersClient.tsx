...
export default function EcosystemBannersClient() {
  ...
  const chooseFile = async (file?: File) => {
    ...
    try {
      ...
      if (inspected.width < 1280 || inspected.height < 695 || aspect < 1.35 || aspect > 4.5) {
      ...
      const targetAspect = 2560 / 695
      const aspectDiff = Math.abs(aspect - targetAspect) / targetAspect
      if (aspectDiff > 0.08) {
        setWarning(`This image (${inspected.width}×${inspected.height}, ${aspect.toFixed(2)}:1) does not match the recommended 3.68:1 ecosystem hero ratio. Some content may be cropped on the public page.`)
      } else {
        setWarning('')
      }
      setError('')
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="rounded-2xl border border-white/10 p-10 text-center text-sm text-white/50">Loading banners...</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((category) => <article key={category.id} className="overflow-hidden rounded-2xl border border-white/10 bg-[#0b1220]">
      ...
      <div className="mx-4 aspect-[2560/695] overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]">{category.banner?.imageUrl ? <img src={category.banner.imageUrl} alt={category.banner.altText} className="h-full w-full object-cover" loading="lazy" /> : <div className="flex h-full items-center justify-center text-xs font-semibold uppercase tracking-[0.16em] text-white/35">No banner uploaded</div>}</div>
      <div className="p-4 text-xs text-white/50">{category.banner ? <>{category.banner.width || '—'} × {category.banner.height || '—'} · {category.banner.mimeType.replace('image/', '').toUpperCase()} · {formatBytes(category.banner.fileSize)}<br />Updated {formatDate(category.banner.updatedAt)}<br />{category.banner.width && category.banner.height ? (Math.abs((category.banner.width / category.banner.height) - (2560 / 695)) / (2560 / 695) <= 0.08 ? <span className="text-emerald-400">OPTIMAL</span> : <span className="text-amber-400">LEGACY DIMENSION · Replace recommended</span>) : null}</> : <>Recommended desktop banner: 2560 × 695<br />Aspect ratio: 3.68:1</>}</div>
